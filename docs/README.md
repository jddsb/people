...
项目当前启动场景为 `Assets/testA/Scenes/testA.scene`，启动脚本为 `Assets/testA/Scripts/TestAWheelRunnerGame.cs`。
...
- 场景中挂载了 `TestAWheelRunnerGame` 组件，运行后由脚本动态生成游戏所需对象。

### 启动脚本

- 脚本路径：`Assets/testA/Scripts/TestAWheelRunnerGame.cs`
- 组件名称：`TestAWheelRunnerGame`
- 生命周期入口：
...
- 轮子颜色与地块颜色不同：轮子高度降低，分数减少但不会低于 0。
...
相关常量位于 `TestAWheelRunnerGame.cs`：
...
- `Assets/testA/Scripts/TestAWheelRunnerGame.cs` 是当前主玩法脚本。
...
在 `TestAWheelRunnerGame` 组件 Inspector 中可以调整以下玩法参数：
...
- 如果只调整玩法数值，优先修改 `TestAWheelRunnerGame` 的序列化字段或常量。
...
确认打开的是 `Assets/testA/Scenes/testA.scene`，并且场景中存在挂载 `TestAWheelRunnerGame` 的对象。
...
`TestAWheelRunnerGame` 会在 `Awake()` 中动态生成灯光、摄像机、跑道、角色和 UI。若需要保留手工摆放对象，需要先调整脚本的生成逻辑。
